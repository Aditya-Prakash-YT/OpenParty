...
- **[Phase 2: The Setup Script](PHASE_2.md)** — The critical `setup.cmd` script for UAC elevation, environment deployment (Python checks, and PowerShell-based installations of VLC, Syncplay, aria2, and qBittorrent), and `.oparty` file association.
...
- **[Phase 5: Create Party (Host Flow)](PHASE_5.md)** — A live-validating form in `gui/create_party.py` to generate `.oparty` JSON configurations. Includes a seamless qBittorrent integration via Web API to automatically create and seed torrents for the host.
